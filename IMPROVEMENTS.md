...
- **Risk:** Low — only Dependabot PRs auto-merge, and only after lint + the full Playwright E2E suite passes. Requires the repo setting *Settings → Actions → General → "Allow GitHub Actions to create and approve pull requests"* to be enabled for the approve step.
